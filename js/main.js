...
(function () {
  ...
  /* ========== 服务标签 hover 图片展示 ========== */
  const tagItems = document.querySelectorAll('.tag-hoverable');
  const photoItems = document.querySelectorAll('.tag-photo-item');
  let hideTimer = null;

  function showTagPhoto(tag) {
    clearTimeout(hideTimer);
    photoItems.forEach(p => {
      if (p.dataset.photo === tag) {
  ...
  function hideTagPhoto() {
    hideTimer = setTimeout(() => {
      photoItems.forEach(p => p.classList.remove('is-active'));
      tagItems.forEach(t => t.classList.remove('tag-active'));
    }, 150);
  }

  tagItems.forEach(tag => {
    tag.addEventListener('mouseenter', () => showTagPhoto(tag.dataset.tag));
    tag.addEventListener('mouseleave', hideTagPhoto);
  });

  // 鼠标移入浮层区域时保持显示
  const overlay = document.getElementById('tagPhotoOverlay');
  if (overlay) {
    overlay.addEventListener('mouseenter', () => clearTimeout(hideTimer));
    overlay.addEventListener('mouseleave', hideTagPhoto);
  }
